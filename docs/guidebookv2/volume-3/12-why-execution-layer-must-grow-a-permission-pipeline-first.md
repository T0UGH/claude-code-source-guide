...
这一组的第一篇不细讲 `allow / deny / ask` 的差异，不展开 Bash 的特殊分析链，也不提前进入 policy limits。它只先立一个前提：
...
## execution 和 permission 为什么是同一条主线的两半

### 前半条主线解决“动作怎样被组织”

卷三 01 到 11 已经把执行层前半条线讲得很清楚：

- `tool_use` 把意图正式化
- orchestration 把调用正式接入
- Tool 把对象形态统一起来
- execution 让现实动作真的发生
- `tool_result` 把现实反馈重新接回主循环

这条线解决的是一个问题：

> **模型意图怎样被稳定地翻译成现实动作。**

### 后半条主线解决“动作在什么边界内被允许发生”

但当执行层真的要稳定运行时，仅有翻译链还不够。

Claude Code 还必须补上另一半：

> **这次已经被组织好的动作，在什么条件下才允许继续落地。**

也就是说，执行层不是只需要一条“把动作做出来”的链，还需要一条“把动作裁进去”的链。

从这个角度看，permission pipeline 不是新主题硬插进卷三，而是执行层自己走到现实接口以后，必然长出来的另一半结构。

### 所以后文不是在讲“执行之外的控制层补丁”，而是在讲执行层自身如何闭合

这也是为什么路线图把权限管线放回卷三，而不是只放到更后面的控制卷里。

因为这里首先要解决的不是产品配置，也不是组织治理，而是执行层最基本的一件事：

> **如果系统已经能碰现实，它靠什么保证“碰现实”本身不是默认放行的。**

这件事不解决，execution runtime 其实还没有真正闭合。
