...
> **在 Claude Code 的权限系统里，`allow / deny / ask` 到底是什么？它们为什么是 runtime 的决策面，而不是 UI 层的三个选项？**
...
## 图 1：`allow / deny / ask` 不是按钮分布，而是控制流分叉面

```mermaid
flowchart TD
    A[某个 tool action 请求] --> B[hooks / rules / classifier / context 汇总判断]
    B --> C{PermissionResult.behavior}
    C -->|allow| D[动作获得执行资格]
    C -->|ask| E[进入交互式裁决分支]
    E --> F[返回结构化决定]
    F --> G[继续执行 / 改写输入 / 终止]
    C -->|deny| H[构造拒绝结果并终止执行]
```
...
### 因为 ask 的意义在于“需要补充裁决”，不在于“采用什么交互媒介”

如果把 `ask` 理解成“弹窗”，会自动带出一个错误前提：
...
用户交互只是这个路径里最常见的一种输入来源。

在设计上，系统真正在意的是：

- 何时从自动放行切到补充裁决
- 补充裁决后如何生成新的 permission result
- 新结果如何回到执行主链

所以 ask 不是“前端名字”，而是“runtime 分支名字”。

### 因为 ask 和 deny 不是强弱关系，而是不同命运

一个常见误解是把 ask 看成“弱 deny”或“暂缓 allow”。
...
它不是 deny 的温和版，也不是 allow 的前奏，而是第三种正式状态。

这说明 Claude Code 的权限系统不是二元世界：
