...
### 因为它返回的不是 yes/no，而是结构化决策对象

这篇不展开 `allow / deny / ask` 的类型学细节，但有一件事必须先立住：

permission decision 不是返回一个布尔值。

如果它只是 true/false，那它对主链能做的事情非常有限：

- true 就继续
- false 就停止

但 Claude Code 里的 permission system 明显不是这个级别。

旧文和现有笔记都指向同一个事实：permission 层会返回结构化结果，里面不只包含“这次怎么判”，还可能带着：
...
这意味着 permission decision 干的不是单纯放行或拦截，而是：

> **把一次工具调用重新写成“接下来应该怎样被执行、被解释、被回传”的版本。**

### `updatedInput` 说明 permission 层不只是守门，还拥有改写权

这里最值得单独指出的是 `updatedInput`。

只要 decision 结果里允许携带更新后的输入，就说明 permission 层并不是一个站在门口只会点头摇头的裁判。

它还可能做另一件更重的事：

> **把原本要执行的调用，改写成一个新的有效调用，再交给 execution。**

于是后面的真正 execution，执行的就不再是 assistant 最初那份输入，而是 permission 层允许继续向下传递的那份输入。

这正是“它会改写后续执行路径”的最直接证据。

### `decisionReason` 说明它还在给后续链路提供解释语义

另一个不能忽略的是 `decisionReason`。

它说明 permission 决策不是黑箱拍板以后就结束了，后面的链路还会关心：

- 这次为什么被这样处理
- 当前结果属于哪类决策来源
- 后续消息或日志该如何解释这次分叉

这意味着 permission decision 不只改写控制流，也在改写这次执行的解释层语义。

### `contentBlocks` 说明 permission 还能影响回流内容的形态

如果 decision 还能携带内容块，那就进一步说明：

> **permission system 不是单纯决定“要不要跑工具”，它还可能参与构造这次调用回给消息层的内容。**

这件事会让 permission 看起来更像 execution 主链的一段，而不是旁边的审批模块。

因为它不只决定是否进入下一步，还直接参与下一步如何被表达。
