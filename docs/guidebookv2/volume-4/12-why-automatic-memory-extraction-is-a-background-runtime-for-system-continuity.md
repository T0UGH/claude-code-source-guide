...
到上一篇为止，我们已经把长期记忆组里最容易混掉的三件事分开了：memory 不是 context 的别名，working memory / transcript / long-term memory 不是一回事，`MEMORY.md` / memdir 也不是普通文件，而是会进入运行时的正式长期记忆层。

但如果只停在这里，读者仍然很容易把长期记忆想成一种“可以被系统读到的记事本”：有载体、有位置、也会被带进来，可本质上仍然像外挂材料。于是自动记忆提取就会被误读成一个附加便利功能——无非是系统帮你省掉手工整理笔记的麻烦。

这正是本篇要收口的地方：**自动记忆提取补上的从来不是“记笔记更方便”这件小事，而是“系统怎样在后台持续筛选、沉淀、回注，让它对用户与项目的认识可以跨任务不断延续”。**
...
## 前八篇解释了“怎么把当前工作接下去”，这一组最后补的是“怎么把认识接下去”

如果回看卷四前面的主线，会发现结构其实很完整。
...
这些问题都成立，但本篇故意不往那个方向膨胀。因为一旦全面展开，文章会迅速偏成“自动化子系统设计说明”，而这不是长期记忆组收口篇的任务。

本篇只需要守住一个更高层的判断：

> **自动记忆提取之所以重要，不在于它有多少调度细节，而在于它把长期记忆从静态对象层推进成了后台运行层。**

也就是说，本篇不负责把 automation 全部讲完，只负责把它在卷四里的架构位置钉死。

## 也不必在这里重新展开所有安全策略分支

同样地，自动记忆提取一定会牵涉边界、作用域、可写范围、记忆污染、错误沉淀等治理问题。这些都是真问题，但如果本篇试图把全部安全策略树状展开，收口篇就会失去焦点。

这里更值得留下的是一个更克制的判断：后台 runtime 并不意味着无边界乱记，相反，**正因为它是后台持续链，它才更需要被边界与治理约束。**

但对这一篇来说，知道这一层就够了。我们不在这里把全部策略分支写成白皮书。
