...
这篇最该先抓住的，不是“plugin 比别的更高级”，而是先把一个很常见的直觉纠正过来。

如果只看表面，plugin 很容易被理解成一个大号扩展包：

- hooks 多一点，外面包一层，叫 plugin
- skills、MCP、commands 放在一起，统称 plugin
- 后面能 install / update / marketplace，于是把它当成产品壳

但 Claude Code 的实现不是这样站位的。

它不是先把 hooks、skills、MCP 各自摆在那里，再统称成“插件世界”；它是先造出一个 `LoadedPlugin` 对象，再把这些不同能力面一项项挂进去。

也就是说，系统先认的是这个对象，后面才认这个对象里装了哪些东西。
