...
## 补图：没有 plugin 时到底会散掉什么

```mermaid
flowchart TD
    A[skills / MCP / hooks / agent 已经存在] --> B{有没有统一封装单位}
    B -->|没有| C[来源散]
    B -->|没有| D[启停散]
    B -->|没有| E[治理散]
    B -->|没有| F[分发散]
...
这张补图不是再画一遍对象关系，而是把第 21 篇真正想回答的问题压成最短缺口图：**前面的扩展入口已经很多，但没有 plugin 时，这整组能力仍然会在来源、启停、治理、分发四处继续散掉。**
