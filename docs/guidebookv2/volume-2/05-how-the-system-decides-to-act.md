...
> **模型什么时候只是继续回答，什么时候会把这一轮推进到能力调用？**

这篇要立住的不是“工具系统很多”，而是一个更准确的判断：

> **Claude Code 判断这一轮要不要继续，不看模型“像不像想做事”，只看 assistant 输出里是否已经出现 runtime 可以接管的 `tool_use` 结构。**

只要这个结构出现，这一轮就不再只是回答，而是已经形成了**当前决策**：下一步该做什么、该把这一轮往哪里推进。
...
```mermaid
flowchart LR
    A[当前工作面: messages + system prompt + 当前 query] --> B[模型流式生成 assistant 输出]
    B --> C[识别输出块]
    C -->|只有 text / thinking| D[回答型产出]
    D --> E[进入收口判断]
    C -->|出现 tool_use| F[行动型产出]
    F --> G[runtime 接住当前决策]
    G --> H[交给执行层]
    H --> I[tool_result 回流]
    I --> J[继续当前 turn]
...
> **不是模型直接做事，而是 assistant 先产出 `tool_use`，runtime 再把这个当前决策继续推进到执行层。**
...
## 关键点四：工具 prompt 也在帮助模型形成这种决策边界

只看 `QueryEngine`，容易觉得是不是模型自己临场决定要不要用能力。

但再看工具 prompt，就会发现系统其实也在前面给边界。

例如：

- `BashTool` prompt 会明确强调优先使用专用工具，不要把所有事都塞给 bash
- `FileReadTool` prompt 会把“读文件”界定成独立能力，而不是让模型靠猜

这些 prompt 的作用，不只是教模型“参数怎么填”，更是在提前塑造判断标准：
...
> **Claude Code 不是让模型随意决定是否调用能力，而是在能力边界上提前提供可学习的行为约束。**

这样，assistant 更容易把“下一步该做什么”落成稳定的 `tool_use` 结构。
...
至于结果回流之后，这一轮为什么继续、为什么停，那是第 07 篇再讲的事。
