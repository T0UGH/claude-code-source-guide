...
flowchart TD
    ...
    C --> D[装配本轮运行时状态]
    D --> E[拉取 system prompt parts]
    E --> F[processUserInput 产出可进入主循环的 messages]
    F --> G[buildSystemInitMessage]
    G --> H[进入 query 主循环]
```
...
如果按时间顺序看，这条线至少有几个关键关口：

1. 输入先被整理，不以裸文本形态进入运行时
2. 请求被正式接入 QueryEngine
3. 当前 query 被组织成可工作的当前工作面
4. 系统在这一轮形成“回答还是行动”的当前判断
5. 如果发生 action / tool use，结果再回流到当前 turn
6. 最后再决定这一轮继续还是收口

所以 QueryEngine 的位置很特殊。它并不直接回答卷二全部问题，但它负责把请求从“前站材料”推进成“主循环里的活动回合”。

```mermaid
flowchart LR
    A[02 输入前处理] --> B[03 进入 QueryEngine]
    B --> C[04 当前 query 的组织]
    C --> D[05 当前决策形成]
    D --> E[06 结果回流]
    E --> F[07 继续还是收口]
...
如果卷二是一条主线，那么第 03 篇要做的，就是把“入口”钉牢。后面第 04、05、06、07 篇讲的，其实都是这扇门后面依次发生的事。
