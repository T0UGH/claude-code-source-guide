...
这一篇当然要回收前文，但不能写成目录说明书。更准确地说，前七篇各自钉住了一段必要责任，到了这里才合成一张完整图。

### 01 先立总图

卷首先说明：主循环必须先作为一条动态主线出现，不能一上来就散进局部机制。到了这里，那张低分辨率总图终于可以被压成正式模型。

### 02 负责输入前站

request 不会裸着进入主循环。它会先经历输入形态识别、材料归位、消息化和 `shouldQuery` 这样的前站分流，因此系统接住的从来不是一段未经处理的原始输入。

### 03 负责正式入口

当前请求随后才被接入 `QueryEngine.submitMessage(...)`。因此 Claude Code 的真正入口是 turn 级入口，不是普通文本入口。

### 04 负责当前工作面

系统真正处理的不是一句孤立文本，而是 current query / 当前工作面：它已经带着 messages、规则和上下文。这一步解释了当前判断为什么总是落在一个被组织好的工作面上。

### 05 负责当前决策

卷二第 05 篇把最关键的分水岭钉住了：assistant 产出的可能是回答，也可能是行动决策；一旦出现 `tool_use`，就说明主循环要切到执行路径。

### 06 负责结果回流

结果回流不是终点，而是把系统送回新的判断点。主循环不是“做过一次动作”就算完成，而是必须把结果重新接回当前 turn。

### 07 负责收口判定

turn 边界也不由“一句话说完了”决定，而要看当前工作是否已经收口。于是卷二最后建立的是：什么时候要继续，什么时候这一轮才真的收住。
