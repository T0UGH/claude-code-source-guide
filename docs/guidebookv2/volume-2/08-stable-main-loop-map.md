...
- **卷内位置**：08 / 08
- **上一篇**：[上一篇：一轮 Agent Turn 什么时候继续，什么时候收口](./07-when-an-agent-turn-continues-or-stops.md)
...
request 不会裸着进入主循环。它先被整理，再被接入 `QueryEngine.submitMessage(...)`，因此 Claude Code 的真正入口是 turn 级入口，不是普通文本入口。
...
结果回流不是终点，而是把系统送回新的判断点；turn 边界也不由“一句话说完了”决定，而要看当前工作是否已经收口。于是卷二后半段最终建立的是：结果如何回流，以及这一轮怎样真正收住。
