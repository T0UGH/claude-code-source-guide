...
这篇只讲进入运行时之前的前站，不抢第 01 篇的总图职责，也不抢第 03 篇“正式进入 QueryEngine”的入口职责。这里真正要讲清的，是：**一条 request 在被正式接入运行时之前，已经先被做成了什么。**
...
flowchart TD
    A[用户输入] --> B[识别输入形态]
    B --> C[处理图片与 pasted contents]
    C --> D[提取附件与外生上下文]
    D --> E[分流 slash / bash / 普通 prompt]
    E --> F[整理成 messages 与运行时附带信息]
    F --> G{shouldQuery?}
    G -- 否 --> H[前站本地收口或转本地处理]
    G -- 是 --> I[作为运行请求进入下一层入口]
```
