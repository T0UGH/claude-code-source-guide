...
> **只要 assistant 已经发起 `tool_use`，这一轮就还没有资格收口。**
...
## 补图：continue / stop 的两道边界

```mermaid
flowchart TD
    A[assistant 当前产出] --> B{有没有 tool_use}
    B -->|有| C[继续
进入执行与结果回流]
    B -->|没有| D[进入收口判定区]
    D --> E{运行时是否同意收口}
...
这张补图把卷二第 07 篇最关键的两道边界直接压出来：**先看模型侧有没有继续动作，再看运行时侧能不能真正收口。** 只有两层都通过，当前 turn 才会停止。
