---
title: 卷二 12｜用一个新会话例子通俗解释 QueryEngine 主链
date: 2026-04-06
tags:
  - Claude Code
  - 源码共读
...
# 卷二 12｜用一个新会话例子通俗解释 QueryEngine 主链

## 导读

- **所属卷**：卷二：一条请求是怎么跑完整个系统的
- **卷内位置**：卷二 12/13
- **上一篇**：[上一篇：用一个新会话例子串起 QueryEngine 主链](./11-queryengine-example-walkthrough.md)
- **下一篇**：[下一篇：system prompt、用户上下文、系统上下文分别是什么](./13-system-user-contexts.md)

这一篇是卷二里偏入门友好的补充篇。它和上一篇谈的是同一条主链，但分工不一样：上一篇强调结构严谨地串链，这一篇则只负责把那条链讲得更顺、更容易第一次读进去。
